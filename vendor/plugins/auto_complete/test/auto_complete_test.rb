...
class AutoCompleteTest < Test::Unit::TestCase
  ...
  def test_auto_complete_field
    ...
    assert_dom_equal %(<script type=\"text/javascript\">\n//<![CDATA[\nvar some_input_auto_completer = new Ajax.Autocompleter('some_input', 'some_input_auto_complete', 'http://www.example.com/autocomplete', {updateElement:function(item){alert('You have chosen: '+item.innerHTML)}})\n//]]>\n</script>),
     auto_complete_field("some_input", :url => { :action => "autocomplete" }, 
            :update_element => "function(item){alert('You have chosen: '+item.innerHTML)}");        
